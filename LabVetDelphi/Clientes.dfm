object FrmClientes: TFrmClientes
  Left = 156
  Top = 85
  Width = 696
  Height = 522
  Caption = 'Cadastro de Cl'#237'nicas Veterin'#225'rias'
  Color = clBtnFace
  Constraints.MaxHeight = 522
  Constraints.MaxWidth = 696
  Constraints.MinHeight = 522
  Constraints.MinWidth = 696
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'Arial'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 16
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 688
    Height = 113
    Align = alTop
    TabOrder = 0
    object Label1: TLabel
      Left = 13
      Top = 28
      Width = 34
      Height = 16
      Caption = 'Nome'
    end
    object EdNome: TEdit
      Left = 56
      Top = 24
      Width = 353
      Height = 24
      TabOrder = 0
    end
    object BbtnPesquisar: TBitBtn
      Left = 576
      Top = 16
      Width = 105
      Height = 33
      Cursor = crHandPoint
      Caption = '&Pesquisar'
      TabOrder = 1
      OnClick = BbtnPesquisarClick
      Glyph.Data = {
        
        5990555555555555577755555555555555555555555555555555}
      NumGlyphs = 2
    end
    object RadioGroup1: TRadioGroup
      Left = 414
      Top = 14
      Width = 139
      Height = 67
      BiDiMode = bdLeftToRight
      Caption = 'Status do Cl'#237'nica'
      Items.Strings = (
        'Ativo'
        'Desativado'
        'Todos')
      ParentBiDiMode = False
      TabOrder = 2
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 113
    Width = 688
    Height = 375
    Align = alClient
    Caption = 'Panel3'
    TabOrder = 1
    object PageControl1: TPageControl
      Left = 1
      Top = 1
      Width = 686
      Height = 373
      ActivePage = TSPesquisa
      Align = alClient
      TabOrder = 0
      object TSPesquisa: TTabSheet
        Caption = 'Pesquisa'
        object Panel2: TPanel
          Left = 573
          Top = 0
          Width = 105
          Height = 342
          Align = alRight
          TabOrder = 0
          object BBtnIncluir: TBitBtn
            Left = 1
            Top = 2
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Incluir'
            TabOrder = 0
            OnClick = BBtnIncluirClick
            Glyph.Data = {
              
              3BB33773333773333773B333333B3333333B7333333733333337}
            NumGlyphs = 2
          end
          object BBtnAlterar: TBitBtn
            Left = 1
            Top = 42
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Alterar'
            TabOrder = 1
            OnClick = BBtnAlterarClick
            Glyph.Data = {
              
              00333377737FFFFF773333303300000003333337337777777333}
            NumGlyphs = 2
          end
          object BbtnExcluir: TBitBtn
            Left = 1
            Top = 82
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Excluir'
            TabOrder = 2
            OnClick = BbtnExcluirClick
            Glyph.Data = {
              
              3333333337FFF7F3333333333000003333333333377777333333}
            NumGlyphs = 2
          end
          object BbtnImprimir: TBitBtn
            Left = 1
            Top = 122
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Imprimir'
            TabOrder = 3
            OnClick = BbtnImprimirClick
            Glyph.Data = {
              
              33333337FFFF7733333333300000033333333337777773333333}
            NumGlyphs = 2
          end
          object BBtnFechat: TBitBtn
            Left = 1
            Top = 162
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Fechar'
            TabOrder = 4
            OnClick = BBtnFechatClick
            Glyph.Data = {
              
              0333337F777FFFFF7F3333000000000003333377777777777333}
            NumGlyphs = 2
          end
        end
        object Panel5: TPanel
          Left = 0
          Top = 0
          Width = 573
          Height = 342
          Align = alClient
          TabOrder = 1
          object DBGrid1: TDBGrid
            Left = 1
            Top = 1
            Width = 571
            Height = 340
            Align = alClient
            Color = clInfoBk
            DataSource = DSQualquer
            TabOrder = 0
            TitleFont.Charset = ANSI_CHARSET
            TitleFont.Color = clWindowText
            TitleFont.Height = -13
            TitleFont.Name = 'Arial'
            TitleFont.Style = []
            Columns = <
              item
                Expanded = False
                FieldName = 'Nome'
                Title.Caption = 'Cl'#237'nica Veterin'#225'ria'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'Veterinario'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'Endereco'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'numero'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'Compl'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'Bairro'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'CEP'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'Cidade'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'UF'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'Telefone'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'Fax'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'Celular'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'email'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'cpf_cgc'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'Contato'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'DataCadastro'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'Restricao'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'DataAtualizacao'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'Ativo'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'CodVetResp'
                Visible = True
              end>
          end
        end
      end
      object TSManutencao: TTabSheet
        Caption = 'Manuten'#231#227'o'
        ImageIndex = 1
        object Panel4: TPanel
          Left = 571
          Top = 0
          Width = 107
          Height = 342
          Align = alRight
          TabOrder = 0
          object BBtnGravar: TBitBtn
            Left = 2
            Top = 2
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Gravar'
            TabOrder = 0
            OnClick = BBtnGravarClick
            Glyph.Data = {
              
              93337FFFF7737777733300000033333333337777773333333333}
            NumGlyphs = 2
          end
          object BBtnCancelar: TBitBtn
            Left = 2
            Top = 42
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Cancelar'
            TabOrder = 1
            OnClick = BBtnCancelarClick
            Glyph.Data = {
              
              3333333777333777333333333999993333333333377777333333}
            NumGlyphs = 2
          end
        end
        object ScrollBox1: TScrollBox
          Left = 0
          Top = 0
          Width = 571
          Height = 342
          Align = alClient
          TabOrder = 1
          object Label2: TLabel
            Left = 8
            Top = 8
            Width = 161
            Height = 16
            Caption = 'Nome da Cl'#237'nica Veterin'#225'ria'
            FocusControl = DBEdit1
          end
          object Label3: TLabel
            Left = 8
            Top = 134
            Width = 55
            Height = 16
            Caption = 'Endere'#231'o'
            FocusControl = DBEdit2
          end
          object Label4: TLabel
            Left = 352
            Top = 134
            Width = 45
            Height = 16
            Caption = 'N'#250'mero'
            FocusControl = DBEdit3
          end
          object Label5: TLabel
            Left = 424
            Top = 134
            Width = 80
            Height = 16
            Caption = 'Complemento'
            FocusControl = DBEdit4
          end
          object Label6: TLabel
            Left = 8
            Top = 174
            Width = 34
            Height = 16
            Caption = 'Bairro'
            FocusControl = DBEdit5
          end
          object Label7: TLabel
            Left = 416
            Top = 214
            Width = 27
            Height = 16
            Caption = 'CEP'
            FocusControl = DBEdit6
          end
          object Label8: TLabel
            Left = 8
            Top = 214
            Width = 40
            Height = 16
            Caption = 'Cidade'
            FocusControl = DBEdit7
          end
          object Label9: TLabel
            Left = 376
            Top = 214
            Width = 17
            Height = 16
            Caption = 'UF'
            FocusControl = DBEdit8
          end
          object Label10: TLabel
            Left = 8
            Top = 254
            Width = 48
            Height = 16
            Caption = 'Telefone'
            FocusControl = DBEdit9
          end
          object Label11: TLabel
            Left = 192
            Top = 254
            Width = 22
            Height = 16
            Caption = 'Fax'
            FocusControl = DBEdit10
          end
          object Label12: TLabel
            Left = 368
            Top = 254
            Width = 40
            Height = 16
            Caption = 'Celular'
            FocusControl = DBEdit11
          end
          object Label13: TLabel
            Left = 8
            Top = 295
            Width = 31
            Height = 16
            Caption = 'email'
            FocusControl = DBEdit12
          end
          object Label14: TLabel
            Left = 216
            Top = 94
            Width = 28
            Height = 16
            Caption = 'CGC'
            FocusControl = DBEdit13
          end
          object Label15: TLabel
            Left = 8
            Top = 94
            Width = 45
            Height = 16
            Caption = 'Contato'
            FocusControl = DBEdit14
          end
          object Label16: TLabel
            Left = 9
            Top = 51
            Width = 108
            Height = 16
            Caption = 'M'#233'dico Veterin'#225'rio'
            FocusControl = DBEdit1
          end
          object DBCheckBox1: TDBCheckBox
            Left = 407
            Top = 3
            Width = 122
            Height = 17
            Caption = 'Restri'#231#227'o'
            DataField = 'Restricao'
            DataSource = DSQualquer
            Font.Charset = ANSI_CHARSET
            Font.Color = clRed
            Font.Height = -13
            Font.Name = 'Arial'
            Font.Style = [fsBold]
            ParentFont = False
            TabOrder = 15
            ValueChecked = 'S'
            ValueUnchecked = 'N'
          end
          object DBEdit1: TDBEdit
            Left = 8
            Top = 24
            Width = 522
            Height = 24
            CharCase = ecUpperCase
            DataField = 'Nome'
            DataSource = DSQualquer
            TabOrder = 0
          end
          object DBEdit2: TDBEdit
            Left = 8
            Top = 150
            Width = 337
            Height = 24
            CharCase = ecUpperCase
            DataField = 'Endereco'
            DataSource = DSQualquer
            TabOrder = 3
          end
          object DBEdit3: TDBEdit
            Left = 352
            Top = 150
            Width = 65
            Height = 24
            CharCase = ecUpperCase
            DataField = 'numero'
            DataSource = DSQualquer
            TabOrder = 4
          end
          object DBEdit4: TDBEdit
            Left = 424
            Top = 150
            Width = 113
            Height = 24
            CharCase = ecUpperCase
            DataField = 'Compl'
            DataSource = DSQualquer
            TabOrder = 5
          end
          object DBEdit5: TDBEdit
            Left = 8
            Top = 190
            Width = 394
            Height = 24
            CharCase = ecUpperCase
            DataField = 'Bairro'
            DataSource = DSQualquer
            TabOrder = 6
          end
          object DBEdit6: TDBEdit
            Left = 416
            Top = 230
            Width = 121
            Height = 24
            DataField = 'CEP'
            DataSource = DSQualquer
            TabOrder = 9
          end
          object DBEdit7: TDBEdit
            Left = 8
            Top = 230
            Width = 361
            Height = 24
            CharCase = ecUpperCase
            DataField = 'Cidade'
            DataSource = DSQualquer
            TabOrder = 7
          end
          object DBEdit8: TDBEdit
            Left = 376
            Top = 230
            Width = 30
            Height = 24
            CharCase = ecUpperCase
            DataField = 'UF'
            DataSource = DSQualquer
            TabOrder = 8
          end
          object DBEdit9: TDBEdit
            Left = 8
            Top = 270
            Width = 173
            Height = 24
            DataField = 'Telefone'
            DataSource = DSQualquer
            TabOrder = 10
          end
          object DBEdit10: TDBEdit
            Left = 192
            Top = 270
            Width = 173
            Height = 24
            DataField = 'Fax'
            DataSource = DSQualquer
            TabOrder = 11
          end
          object DBEdit11: TDBEdit
            Left = 368
            Top = 270
            Width = 169
            Height = 24
            DataField = 'Celular'
            DataSource = DSQualquer
            TabOrder = 12
          end
          object DBEdit12: TDBEdit
            Left = 8
            Top = 311
            Width = 529
            Height = 24
            CharCase = ecLowerCase
            DataField = 'email'
            DataSource = DSQualquer
            TabOrder = 13
          end
          object DBEdit13: TDBEdit
            Left = 216
            Top = 110
            Width = 238
            Height = 24
            DataField = 'cpf_cgc'
            DataSource = DSQualquer
            TabOrder = 2
          end
          object DBEdit14: TDBEdit
            Left = 8
            Top = 110
            Width = 199
            Height = 24
            CharCase = ecUpperCase
            DataField = 'Contato'
            DataSource = DSQualquer
            TabOrder = 1
          end
          object DBCheckBox2: TDBCheckBox
            Left = 271
            Top = 3
            Width = 97
            Height = 17
            Caption = 'Ativo'
            DataField = 'ativo'
            DataSource = DSQualquer
            Font.Charset = ANSI_CHARSET
            Font.Color = clRed
            Font.Height = -13
            Font.Name = 'Arial'
            Font.Style = [fsBold]
            ParentFont = False
            TabOrder = 14
            ValueChecked = 'S'
            ValueUnchecked = 'N'
          end
          object RxDBLookupCombo1: TRxDBLookupCombo
            Left = 9
            Top = 67
            Width = 520
            Height = 23
            DropDownCount = 8
            DataField = 'CodVetResp'
            DataSource = DSQualquer
            LookupField = 'CodProfSolic'
            LookupDisplay = 'NomeProfSolic'
            LookupSource = DSVeterinario
            TabOrder = 16
          end
        end
      end
    end
  end
  object QryQualquer: TQuery
    CachedUpdates = True
    DatabaseName = 'DNLABVET'
    SQL.Strings = (
      'Select c.*, p.nomeprofsolic as Veterinario'
      
        'From Cliente c left join profissionalsolicitante p on c.codvetre' +
        'sp = p.codprofsolic'
      'Where c.Nome Like :Nome'
      'and'
      '(c.ativo = :status or :status = '#39'-1'#39')'
      'Order by c.Nome')
    UpdateObject = UDPQualquer
    Left = 341
    Top = 125
    ParamData = <
      item
        DataType = ftString
        Name = 'Nome'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'status'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'status'
        ParamType = ptInput
      end>
    object QryQualquerCodCliente: TAutoIncField
      FieldName = 'CodCliente'
      Origin = 'DNLABVET.Cliente.CodCliente'
    end
    object QryQualquerNome: TStringField
      FieldName = 'Nome'
      Origin = 'DNLABVET.Cliente.Nome'
      FixedChar = True
      Size = 50
    end
    object QryQualquerEndereco: TStringField
      FieldName = 'Endereco'
      Origin = 'DNLABVET.Cliente.Endereco'
      FixedChar = True
      Size = 40
    end
    object QryQualquernumero: TStringField
      FieldName = 'numero'
      Origin = 'DNLABVET.Cliente.numero'
      FixedChar = True
      Size = 50
    end
    object QryQualquerCompl: TStringField
      FieldName = 'Compl'
      Origin = 'DNLABVET.Cliente.Compl'
      FixedChar = True
    end
    object QryQualquerBairro: TStringField
      FieldName = 'Bairro'
      Origin = 'DNLABVET.Cliente.Bairro'
      FixedChar = True
      Size = 30
    end
    object QryQualquerCEP: TStringField
      FieldName = 'CEP'
      Origin = 'DNLABVET.Cliente.CEP'
      EditMask = '00000\-999;1;_'
      FixedChar = True
      Size = 9
    end
    object QryQualquerCidade: TStringField
      FieldName = 'Cidade'
      Origin = 'DNLABVET.Cliente.Cidade'
      FixedChar = True
      Size = 30
    end
    object QryQualquerUF: TStringField
      FieldName = 'UF'
      Origin = 'DNLABVET.Cliente.UF'
      FixedChar = True
      Size = 2
    end
    object QryQualquerTelefone: TStringField
      FieldName = 'Telefone'
      Origin = 'DNLABVET.Cliente.Telefone'
      EditMask = '!\(99\)0000-0000;1;_'
      FixedChar = True
      Size = 13
    end
    object QryQualquerFax: TStringField
      FieldName = 'Fax'
      Origin = 'DNLABVET.Cliente.Fax'
      EditMask = '!\(99\)0000-0000;1;_'
      FixedChar = True
      Size = 13
    end
    object QryQualquerCelular: TStringField
      FieldName = 'Celular'
      Origin = 'DNLABVET.Cliente.Celular'
      EditMask = '!\(99\)0000-0000;1;_'
      FixedChar = True
      Size = 13
    end
    object QryQualqueremail: TStringField
      FieldName = 'email'
      Origin = 'DNLABVET.Cliente.email'
      FixedChar = True
      Size = 200
    end
    object QryQualquercpf_cgc: TStringField
      FieldName = 'cpf_cgc'
      Origin = 'DNLABVET.Cliente.cpf_cgc'
      FixedChar = True
      Size = 18
    end
    object QryQualquerContato: TStringField
      FieldName = 'Contato'
      Origin = 'DNLABVET.Cliente.Contato'
      FixedChar = True
      Size = 15
    end
    object QryQualquerDataCadastro: TDateTimeField
      FieldName = 'DataCadastro'
      Origin = 'DNLABVET.Cliente.DataCadastro'
    end
    object QryQualquerRestricao: TStringField
      FieldName = 'Restricao'
      Origin = 'DNLABVET.Cliente.Restricao'
      FixedChar = True
      Size = 1
    end
    object QryQualquerDataAtualizacao: TDateTimeField
      FieldName = 'DataAtualizacao'
      Origin = 'DNLABVET.Cliente.DataAtualizacao'
    end
    object QryQualquerAtivo: TStringField
      FieldName = 'Ativo'
      Origin = 'DNLABVET.Cliente.Ativo'
      FixedChar = True
      Size = 1
    end
    object QryQualquerCodVetResp: TIntegerField
      FieldName = 'CodVetResp'
      Origin = 'DNLABVET.Cliente.CodVetResp'
    end
    object QryQualquerVeterinario: TStringField
      FieldName = 'Veterinario'
      FixedChar = True
      Size = 50
    end
  end
  object UDPQualquer: TUpdateSQL
    ModifySQL.Strings = (
      'update Cliente'
      'set'
      '  Nome = :Nome,'
      '  Endereco = :Endereco,'
      '  numero = :numero,'
      '  Compl = :Compl,'
      '  Bairro = :Bairro,'
      '  CEP = :CEP,'
      '  Cidade = :Cidade,'
      '  UF = :UF,'
      '  Telefone = :Telefone,'
      '  Fax = :Fax,'
      '  Celular = :Celular,'
      '  email = :email,'
      '  cpf_cgc = :cpf_cgc,'
      '  Contato = :Contato,'
      '  DataCadastro = :DataCadastro,'
      '  Restricao = :Restricao,'
      '  DataAtualizacao = :DataAtualizacao,'
      '  Ativo = :Ativo,'
      '  CodVetResp = :CodVetResp'
      'where'
      '  CodCliente = :OLD_CodCliente')
    InsertSQL.Strings = (
      'insert into Cliente'
      
        '  (Nome, Endereco, numero, Compl, Bairro, CEP, Cidade, UF, Telef' +
        'one, Fax, '
      
        '   Celular, email, cpf_cgc, Contato, DataCadastro, Restricao, Da' +
        'taAtualizacao, '
      '   Ativo, CodVetResp)'
      'values'
      
        '  (:Nome, :Endereco, :numero, :Compl, :Bairro, :CEP, :Cidade, :U' +
        'F, :Telefone, '
      
        '   :Fax, :Celular, :email, :cpf_cgc, :Contato, :DataCadastro, :R' +
        'estricao, '
      '   :DataAtualizacao, :Ativo, :CodVetResp)')
    DeleteSQL.Strings = (
      'delete from Cliente'
      'where'
      '  CodCliente = :OLD_CodCliente')
    Left = 389
    Top = 165
  end
  object DSQualquer: TDataSource
    AutoEdit = False
    DataSet = QryQualquer
    Left = 421
    Top = 125
  end
  object QryVeterinario: TQuery
    DatabaseName = 'DNLABVET'
    SQL.Strings = (
      'Select *'
      'From ProfissionalSolicitante'
      'order by nomeprofsolic')
    Left = 461
    Top = 253
    object QryVeterinarioCodProfSolic: TAutoIncField
      FieldName = 'CodProfSolic'
      Origin = 'DNLABVET.ProfissionalSolicitante.CodProfSolic'
    end
    object QryVeterinarioAbrevConselho: TStringField
      FieldName = 'AbrevConselho'
      Origin = 'DNLABVET.ProfissionalSolicitante.AbrevConselho'
      FixedChar = True
      Size = 10
    end
    object QryVeterinarioNomeProfSolic: TStringField
      FieldName = 'NomeProfSolic'
      Origin = 'DNLABVET.ProfissionalSolicitante.NomeProfSolic'
      FixedChar = True
      Size = 50
    end
    object QryVeterinarioNumConselho: TStringField
      FieldName = 'NumConselho'
      Origin = 'DNLABVET.ProfissionalSolicitante.NumConselho'
      FixedChar = True
      Size = 15
    end
    object QryVeterinarioEmail: TStringField
      FieldName = 'Email'
      Origin = 'DNLABVET.ProfissionalSolicitante.Email'
      FixedChar = True
      Size = 200
    end
    object QryVeterinarioTelefone: TStringField
      FieldName = 'Telefone'
      Origin = 'DNLABVET.ProfissionalSolicitante.Telefone'
      FixedChar = True
      Size = 15
    end
    object QryVeterinarioCelular: TStringField
      FieldName = 'Celular'
      Origin = 'DNLABVET.ProfissionalSolicitante.Celular'
      FixedChar = True
      Size = 15
    end
    object QryVeterinarioNascimento: TDateTimeField
      FieldName = 'Nascimento'
      Origin = 'DNLABVET.ProfissionalSolicitante.Nascimento'
    end
    object QryVeterinarioativo: TStringField
      FieldName = 'ativo'
      Origin = 'DNLABVET.ProfissionalSolicitante.ativo'
      FixedChar = True
      Size = 1
    end
  end
  object DSVeterinario: TDataSource
    AutoEdit = False
    DataSet = QryVeterinario
    Left = 501
    Top = 253
  end
end
