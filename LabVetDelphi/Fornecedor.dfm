object FrmFornecedor: TFrmFornecedor
  Left = 158
  Top = 114
  Width = 696
  Height = 500
  Caption = 'Fornecedores'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 688
    Height = 113
    Align = alTop
    TabOrder = 0
    object Label1: TLabel
      Left = 13
      Top = 28
      Width = 28
      Height = 13
      Caption = 'Nome'
    end
    object EdNome: TEdit
      Left = 56
      Top = 24
      Width = 353
      Height = 21
      TabOrder = 0
    end
    object BbtnPesquisar: TBitBtn
      Left = 576
      Top = 16
      Width = 105
      Height = 33
      Cursor = crHandPoint
      Caption = '&Pesquisar'
      TabOrder = 1
      OnClick = BbtnPesquisarClick
      Glyph.Data = {
        
        5990555555555555577755555555555555555555555555555555}
      NumGlyphs = 2
    end
    object RadioGroup1: TRadioGroup
      Left = 414
      Top = 14
      Width = 139
      Height = 67
      BiDiMode = bdLeftToRight
      Caption = 'Status do Cl'#237'nica'
      Items.Strings = (
        'Ativo'
        'Desativado'
        'Todos')
      ParentBiDiMode = False
      TabOrder = 2
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 113
    Width = 688
    Height = 353
    Align = alClient
    Caption = 'Panel3'
    TabOrder = 1
    object PageControl1: TPageControl
      Left = 1
      Top = 1
      Width = 686
      Height = 351
      ActivePage = TSManutencao
      Align = alClient
      TabOrder = 0
      object TSPesquisa: TTabSheet
        Caption = 'Pesquisa'
        object Panel2: TPanel
          Left = 573
          Top = 0
          Width = 105
          Height = 323
          Align = alRight
          TabOrder = 0
          object BBtnIncluir: TBitBtn
            Left = 1
            Top = 2
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Incluir'
            TabOrder = 0
            OnClick = BBtnIncluirClick
            Glyph.Data = {
              
              3BB33773333773333773B333333B3333333B7333333733333337}
            NumGlyphs = 2
          end
          object BBtnAlterar: TBitBtn
            Left = 1
            Top = 42
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Alterar'
            TabOrder = 1
            OnClick = BBtnAlterarClick
            Glyph.Data = {
              
              00333377737FFFFF773333303300000003333337337777777333}
            NumGlyphs = 2
          end
          object BbtnExcluir: TBitBtn
            Left = 1
            Top = 82
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Excluir'
            TabOrder = 2
            OnClick = BbtnExcluirClick
            Glyph.Data = {
              
              3333333337FFF7F3333333333000003333333333377777333333}
            NumGlyphs = 2
          end
          object BbtnImprimir: TBitBtn
            Left = 1
            Top = 122
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Imprimir'
            TabOrder = 3
            OnClick = BbtnImprimirClick
            Glyph.Data = {
              
              33333337FFFF7733333333300000033333333337777773333333}
            NumGlyphs = 2
          end
          object BBtnFechat: TBitBtn
            Left = 1
            Top = 162
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Fechar'
            TabOrder = 4
            OnClick = BBtnFechatClick
            Glyph.Data = {
              
              0333337F777FFFFF7F3333000000000003333377777777777333}
            NumGlyphs = 2
          end
        end
        object Panel5: TPanel
          Left = 0
          Top = 0
          Width = 573
          Height = 323
          Align = alClient
          TabOrder = 1
          object DBGrid1: TDBGrid
            Left = 1
            Top = 1
            Width = 571
            Height = 321
            Align = alClient
            Color = clInfoBk
            DataSource = DSQualquer
            TabOrder = 0
            TitleFont.Charset = DEFAULT_CHARSET
            TitleFont.Color = clWindowText
            TitleFont.Height = -11
            TitleFont.Name = 'MS Sans Serif'
            TitleFont.Style = []
            Columns = <
              item
                Expanded = False
                FieldName = 'NOMEFORN'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'APELIDO'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'FONE'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'FONE1'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'FONE_FAX'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'CGC'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'DTCADASTRO'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'CODIGO'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'INSCRICAO'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'ENDERECO'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'BAIRRO'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'CIDADE'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'CEP'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'UF'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'EMAIL'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'CONTATO'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'Ativo'
                Visible = True
              end>
          end
        end
      end
      object TSManutencao: TTabSheet
        Caption = 'Manuten'#231#227'o'
        ImageIndex = 1
        object Panel4: TPanel
          Left = 571
          Top = 0
          Width = 107
          Height = 323
          Align = alRight
          TabOrder = 0
          object BBtnGravar: TBitBtn
            Left = 2
            Top = 2
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Gravar'
            TabOrder = 0
            OnClick = BBtnGravarClick
            Glyph.Data = {
              
              93337FFFF7737777733300000033333333337777773333333333}
            NumGlyphs = 2
          end
          object BBtnCancelar: TBitBtn
            Left = 2
            Top = 42
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Cancelar'
            TabOrder = 1
            OnClick = BBtnCancelarClick
            Glyph.Data = {
              
              3333333777333777333333333999993333333333377777333333}
            NumGlyphs = 2
          end
        end
        object ScrollBox1: TScrollBox
          Left = 0
          Top = 0
          Width = 571
          Height = 323
          Align = alClient
          TabOrder = 1
          object Label2: TLabel
            Left = 8
            Top = 8
            Width = 54
            Height = 13
            Caption = 'Fornecedor'
            FocusControl = DBEdit1
          end
          object Label3: TLabel
            Left = 8
            Top = 48
            Width = 35
            Height = 13
            Caption = 'Apelido'
            FocusControl = DBEdit2
          end
          object Label4: TLabel
            Left = 8
            Top = 88
            Width = 27
            Height = 13
            Caption = 'CNPJ'
            FocusControl = DBEdit3
          end
          object Label7: TLabel
            Left = 280
            Top = 88
            Width = 43
            Height = 13
            Caption = 'Inscri'#231#227'o'
            FocusControl = DBEdit6
          end
          object Label8: TLabel
            Left = 8
            Top = 128
            Width = 64
            Height = 13
            Caption = 'Endere'#231'o, N'#186
            FocusControl = DBEdit7
          end
          object Label9: TLabel
            Left = 320
            Top = 128
            Width = 27
            Height = 13
            Caption = 'Bairro'
            FocusControl = DBEdit8
          end
          object Label10: TLabel
            Left = 8
            Top = 168
            Width = 33
            Height = 13
            Caption = 'Cidade'
            FocusControl = DBEdit9
          end
          object Label11: TLabel
            Left = 320
            Top = 168
            Width = 21
            Height = 13
            Caption = 'CEP'
            FocusControl = DBEdit10
          end
          object Label12: TLabel
            Left = 416
            Top = 168
            Width = 14
            Height = 13
            Caption = 'UF'
            FocusControl = DBEdit11
          end
          object Label13: TLabel
            Left = 8
            Top = 208
            Width = 42
            Height = 13
            Caption = 'Telefone'
            FocusControl = DBEdit12
          end
          object Label14: TLabel
            Left = 144
            Top = 208
            Width = 32
            Height = 13
            Caption = 'Celular'
            FocusControl = DBEdit13
          end
          object Label15: TLabel
            Left = 288
            Top = 208
            Width = 70
            Height = 13
            Caption = 'Telefone / Fax'
            FocusControl = DBEdit14
          end
          object Label16: TLabel
            Left = 8
            Top = 256
            Width = 25
            Height = 13
            Caption = 'Email'
            FocusControl = DBEdit15
          end
          object Label17: TLabel
            Left = 216
            Top = 48
            Width = 37
            Height = 13
            Caption = 'Contato'
            FocusControl = DBEdit16
          end
          object Label5: TLabel
            Left = 416
            Top = 48
            Width = 142
            Height = 13
            Caption = 'Dia Vencimento se Conta Fixa'
          end
          object DBCheckBox2: TDBCheckBox
            Left = 271
            Top = 3
            Width = 97
            Height = 17
            Caption = 'Ativo'
            DataField = 'ativo'
            DataSource = DSQualquer
            Font.Charset = ANSI_CHARSET
            Font.Color = clRed
            Font.Height = -13
            Font.Name = 'Arial'
            Font.Style = [fsBold]
            ParentFont = False
            TabOrder = 14
            ValueChecked = 'S'
            ValueUnchecked = 'N'
          end
          object DBEdit1: TDBEdit
            Left = 8
            Top = 24
            Width = 535
            Height = 21
            CharCase = ecUpperCase
            DataField = 'NOMEFORN'
            DataSource = DSQualquer
            TabOrder = 0
          end
          object DBEdit2: TDBEdit
            Left = 8
            Top = 64
            Width = 199
            Height = 21
            CharCase = ecUpperCase
            DataField = 'APELIDO'
            DataSource = DSQualquer
            TabOrder = 1
          end
          object DBEdit3: TDBEdit
            Left = 8
            Top = 104
            Width = 264
            Height = 21
            DataField = 'CGC'
            DataSource = DSQualquer
            TabOrder = 3
          end
          object DBEdit6: TDBEdit
            Left = 280
            Top = 104
            Width = 264
            Height = 21
            DataField = 'INSCRICAO'
            DataSource = DSQualquer
            TabOrder = 4
          end
          object DBEdit7: TDBEdit
            Left = 8
            Top = 144
            Width = 305
            Height = 21
            CharCase = ecUpperCase
            DataField = 'ENDERECO'
            DataSource = DSQualquer
            TabOrder = 5
          end
          object DBEdit8: TDBEdit
            Left = 320
            Top = 144
            Width = 224
            Height = 21
            CharCase = ecUpperCase
            DataField = 'BAIRRO'
            DataSource = DSQualquer
            TabOrder = 6
          end
          object DBEdit9: TDBEdit
            Left = 8
            Top = 184
            Width = 306
            Height = 21
            CharCase = ecUpperCase
            DataField = 'CIDADE'
            DataSource = DSQualquer
            TabOrder = 7
          end
          object DBEdit10: TDBEdit
            Left = 320
            Top = 184
            Width = 89
            Height = 21
            DataField = 'CEP'
            DataSource = DSQualquer
            TabOrder = 8
          end
          object DBEdit11: TDBEdit
            Left = 416
            Top = 184
            Width = 30
            Height = 21
            CharCase = ecUpperCase
            DataField = 'UF'
            DataSource = DSQualquer
            TabOrder = 9
          end
          object DBEdit12: TDBEdit
            Left = 8
            Top = 224
            Width = 129
            Height = 21
            DataField = 'FONE'
            DataSource = DSQualquer
            TabOrder = 10
          end
          object DBEdit13: TDBEdit
            Left = 144
            Top = 224
            Width = 137
            Height = 21
            DataField = 'FONE1'
            DataSource = DSQualquer
            TabOrder = 11
          end
          object DBEdit14: TDBEdit
            Left = 288
            Top = 224
            Width = 199
            Height = 21
            DataField = 'FONE_FAX'
            DataSource = DSQualquer
            TabOrder = 12
          end
          object DBEdit15: TDBEdit
            Left = 8
            Top = 272
            Width = 503
            Height = 21
            DataField = 'EMAIL'
            DataSource = DSQualquer
            TabOrder = 13
          end
          object DBEdit16: TDBEdit
            Left = 216
            Top = 64
            Width = 193
            Height = 21
            CharCase = ecUpperCase
            DataField = 'CONTATO'
            DataSource = DSQualquer
            TabOrder = 2
          end
          object SpinEdit1: TSpinEdit
            Left = 416
            Top = 64
            Width = 121
            Height = 22
            MaxValue = 31
            MinValue = 0
            TabOrder = 15
            Value = 0
          end
        end
      end
    end
  end
  object QryQualquer: TQuery
    CachedUpdates = True
    DatabaseName = 'DNLABVET'
    SQL.Strings = (
      'Select *'
      'From Fornecedor'
      'Where NomeForn Like :Nome'
      'and'
      '(ativo = :status or :status = '#39'-1'#39')'
      'Order by NomeForn')
    UpdateObject = UDPQualquer
    Left = 341
    Top = 125
    ParamData = <
      item
        DataType = ftString
        Name = 'Nome'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'status'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'status'
        ParamType = ptInput
      end>
    object QryQualquerCODFORN: TAutoIncField
      FieldName = 'CODFORN'
      Origin = 'DNLABVET.Fornecedor.CODFORN'
    end
    object QryQualquerNOMEFORN: TStringField
      FieldName = 'NOMEFORN'
      Origin = 'DNLABVET.Fornecedor.NOMEFORN'
      FixedChar = True
      Size = 50
    end
    object QryQualquerAPELIDO: TStringField
      FieldName = 'APELIDO'
      Origin = 'DNLABVET.Fornecedor.APELIDO'
      FixedChar = True
      Size = 15
    end
    object QryQualquerCGC: TStringField
      FieldName = 'CGC'
      Origin = 'DNLABVET.Fornecedor.CGC'
      FixedChar = True
    end
    object QryQualquerDTCADASTRO: TDateTimeField
      FieldName = 'DTCADASTRO'
      Origin = 'DNLABVET.Fornecedor.DTCADASTRO'
    end
    object QryQualquerCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Origin = 'DNLABVET.Fornecedor.CODIGO'
    end
    object QryQualquerINSCRICAO: TStringField
      FieldName = 'INSCRICAO'
      Origin = 'DNLABVET.Fornecedor.INSCRICAO'
      FixedChar = True
    end
    object QryQualquerENDERECO: TStringField
      FieldName = 'ENDERECO'
      Origin = 'DNLABVET.Fornecedor.ENDERECO'
      FixedChar = True
      Size = 60
    end
    object QryQualquerBAIRRO: TStringField
      FieldName = 'BAIRRO'
      Origin = 'DNLABVET.Fornecedor.BAIRRO'
      FixedChar = True
      Size = 30
    end
    object QryQualquerCIDADE: TStringField
      FieldName = 'CIDADE'
      Origin = 'DNLABVET.Fornecedor.CIDADE'
      FixedChar = True
      Size = 50
    end
    object QryQualquerCEP: TStringField
      FieldName = 'CEP'
      Origin = 'DNLABVET.Fornecedor.CEP'
      FixedChar = True
      Size = 15
    end
    object QryQualquerUF: TStringField
      FieldName = 'UF'
      Origin = 'DNLABVET.Fornecedor.UF'
      FixedChar = True
      Size = 2
    end
    object QryQualquerFONE: TStringField
      FieldName = 'FONE'
      Origin = 'DNLABVET.Fornecedor.FONE'
      EditMask = '!\(99\)0000-0000;1;_'
      FixedChar = True
      Size = 15
    end
    object QryQualquerFONE1: TStringField
      FieldName = 'FONE1'
      Origin = 'DNLABVET.Fornecedor.FONE1'
      EditMask = '!\(99\)0000-0000;1;_'
      FixedChar = True
      Size = 15
    end
    object QryQualquerFONE_FAX: TStringField
      FieldName = 'FONE_FAX'
      Origin = 'DNLABVET.Fornecedor.FONE_FAX'
      EditMask = '!\(99\)0000-0000;1;_'
      FixedChar = True
      Size = 15
    end
    object QryQualquerEMAIL: TStringField
      FieldName = 'EMAIL'
      Origin = 'DNLABVET.Fornecedor.EMAIL'
      FixedChar = True
      Size = 100
    end
    object QryQualquerCONTATO: TStringField
      FieldName = 'CONTATO'
      Origin = 'DNLABVET.Fornecedor.CONTATO'
      FixedChar = True
    end
    object QryQualquerAtivo: TStringField
      FieldName = 'Ativo'
      Origin = 'DNLABVET.Fornecedor.Ativo'
      FixedChar = True
      Size = 1
    end
    object QryQualquerDIA_VENC: TIntegerField
      FieldName = 'DIA_VENC'
      Origin = 'DNLABVET.Fornecedor.DIA_VENC'
    end
  end
  object UDPQualquer: TUpdateSQL
    ModifySQL.Strings = (
      'update Fornecedor'
      'set'
      '  NOMEFORN = :NOMEFORN,'
      '  APELIDO = :APELIDO,'
      '  DIA_VENC = :DIA_VENC,'
      '  CGC = :CGC,'
      '  DTCADASTRO = :DTCADASTRO,'
      '  CODIGO = :CODIGO,'
      '  INSCRICAO = :INSCRICAO,'
      '  ENDERECO = :ENDERECO,'
      '  BAIRRO = :BAIRRO,'
      '  CIDADE = :CIDADE,'
      '  CEP = :CEP,'
      '  UF = :UF,'
      '  FONE = :FONE,'
      '  FONE1 = :FONE1,'
      '  FONE_FAX = :FONE_FAX,'
      '  EMAIL = :EMAIL,'
      '  CONTATO = :CONTATO,'
      '  Ativo = :Ativo'
      'where'
      '  CODFORN = :OLD_CODFORN')
    InsertSQL.Strings = (
      'insert into Fornecedor'
      
        '  (NOMEFORN, APELIDO, DIA_VENC, CGC, DTCADASTRO, CODIGO, INSCRIC' +
        'AO, ENDERECO, '
      
        '   BAIRRO, CIDADE, CEP, UF, FONE, FONE1, FONE_FAX, EMAIL, CONTAT' +
        'O, Ativo)'
      'values'
      
        '  (:NOMEFORN, :APELIDO, :DIA_VENC, :CGC, :DTCADASTRO, :CODIGO, :' +
        'INSCRICAO, '
      
        '   :ENDERECO, :BAIRRO, :CIDADE, :CEP, :UF, :FONE, :FONE1, :FONE_' +
        'FAX, :EMAIL, '
      '   :CONTATO, :Ativo)')
    DeleteSQL.Strings = (
      'delete from Fornecedor'
      'where'
      '  CODFORN = :OLD_CODFORN')
    Left = 389
    Top = 165
  end
  object DSQualquer: TDataSource
    AutoEdit = False
    DataSet = QryQualquer
    Left = 421
    Top = 125
  end
end
