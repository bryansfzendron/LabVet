object FrmProfSolic: TFrmProfSolic
  Left = 202
  Top = 210
  Width = 681
  Height = 378
  Caption = 'Cadastro de M'#233'dicos Veterin'#225'rios'
  Color = clBtnFace
  Constraints.MaxHeight = 378
  Constraints.MaxWidth = 681
  Constraints.MinHeight = 378
  Constraints.MinWidth = 681
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'Arial'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 16
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 673
    Height = 105
    Align = alTop
    TabOrder = 0
    object Label1: TLabel
      Left = 13
      Top = 28
      Width = 34
      Height = 16
      Caption = 'Nome'
    end
    object EdNome: TEdit
      Left = 56
      Top = 24
      Width = 353
      Height = 24
      TabOrder = 0
    end
    object BbtnPesquisar: TBitBtn
      Left = 560
      Top = 16
      Width = 105
      Height = 33
      Cursor = crHandPoint
      Caption = '&Pesquisar'
      TabOrder = 1
      OnClick = BbtnPesquisarClick
      Glyph.Data = {
        
        5990555555555555577755555555555555555555555555555555}
      NumGlyphs = 2
    end
    object RadioGroup1: TRadioGroup
      Left = 414
      Top = 14
      Width = 139
      Height = 67
      BiDiMode = bdLeftToRight
      Caption = 'Status do Profissional'
      Items.Strings = (
        'Ativo'
        'Desativado'
        'Todos')
      ParentBiDiMode = False
      TabOrder = 2
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 105
    Width = 673
    Height = 239
    Align = alClient
    Caption = 'Panel3'
    TabOrder = 1
    object PageControl1: TPageControl
      Left = 1
      Top = 1
      Width = 671
      Height = 237
      ActivePage = TSPesquisa
      Align = alClient
      TabOrder = 0
      object TSPesquisa: TTabSheet
        Caption = 'Pesquisa'
        object Panel2: TPanel
          Left = 558
          Top = 0
          Width = 105
          Height = 206
          Align = alRight
          TabOrder = 0
          object BBtnIncluir: TBitBtn
            Left = 1
            Top = 2
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Incluir'
            TabOrder = 0
            OnClick = BBtnIncluirClick
            Glyph.Data = {
              
              3BB33773333773333773B333333B3333333B7333333733333337}
            NumGlyphs = 2
          end
          object BBtnAlterar: TBitBtn
            Left = 1
            Top = 42
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Alterar'
            TabOrder = 1
            OnClick = BBtnAlterarClick
            Glyph.Data = {
              
              00333377737FFFFF773333303300000003333337337777777333}
            NumGlyphs = 2
          end
          object BbtnExcluir: TBitBtn
            Left = 1
            Top = 82
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Excluir'
            TabOrder = 2
            OnClick = BbtnExcluirClick
            Glyph.Data = {
              
              3333333337FFF7F3333333333000003333333333377777333333}
            NumGlyphs = 2
          end
          object BbtnImprimir: TBitBtn
            Left = 1
            Top = 122
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Imprimir'
            TabOrder = 3
            OnClick = BbtnImprimirClick
            Glyph.Data = {
              
              33333337FFFF7733333333300000033333333337777773333333}
            NumGlyphs = 2
          end
          object BBtnFechat: TBitBtn
            Left = 1
            Top = 162
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Fechar'
            TabOrder = 4
            OnClick = BBtnFechatClick
            Glyph.Data = {
              
              0333337F777FFFFF7F3333000000000003333377777777777333}
            NumGlyphs = 2
          end
        end
        object Panel5: TPanel
          Left = 0
          Top = 0
          Width = 558
          Height = 206
          Align = alClient
          TabOrder = 1
          object DBGrid1: TDBGrid
            Left = 1
            Top = 1
            Width = 556
            Height = 204
            Align = alClient
            Color = clInfoBk
            DataSource = DSQualquer
            TabOrder = 0
            TitleFont.Charset = ANSI_CHARSET
            TitleFont.Color = clWindowText
            TitleFont.Height = -13
            TitleFont.Name = 'Arial'
            TitleFont.Style = []
            Columns = <
              item
                Expanded = False
                FieldName = 'NomeProfSolic'
                Title.Caption = 'M'#233'dico Veterin'#225'rio'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'NumConselho'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'AbrevConselho'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'Email'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'Telefone'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'Celular'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'Nascimento'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'ativo'
                Visible = True
              end>
          end
        end
      end
      object TSManutencao: TTabSheet
        Caption = 'Manuten'#231#227'o'
        ImageIndex = 1
        object Label2: TLabel
          Left = 16
          Top = 24
          Width = 108
          Height = 16
          Caption = 'M'#233'dico Veterin'#225'rio'
          FocusControl = DBEdit1
        end
        object Label3: TLabel
          Left = 168
          Top = 71
          Width = 121
          Height = 16
          Caption = 'N'#250'mero do Conselho'
          FocusControl = DBEdit2
        end
        object Label4: TLabel
          Left = 16
          Top = 72
          Width = 54
          Height = 16
          Caption = 'Conselho'
        end
        object Label5: TLabel
          Left = 16
          Top = 160
          Width = 33
          Height = 16
          Caption = 'Email'
          FocusControl = DBEdit3
        end
        object Label6: TLabel
          Left = 16
          Top = 115
          Width = 48
          Height = 16
          Caption = 'Telefone'
          FocusControl = DBEdit4
        end
        object Label7: TLabel
          Left = 200
          Top = 115
          Width = 40
          Height = 16
          Caption = 'Celular'
          FocusControl = DBEdit5
        end
        object Label8: TLabel
          Left = 378
          Top = 69
          Width = 69
          Height = 16
          Caption = 'Nascimento'
        end
        object Panel4: TPanel
          Left = 556
          Top = 0
          Width = 107
          Height = 206
          Align = alRight
          TabOrder = 8
          object BBtnGravar: TBitBtn
            Left = 2
            Top = 2
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Gravar'
            TabOrder = 0
            OnClick = BBtnGravarClick
            Glyph.Data = {
              
              93337FFFF7737777733300000033333333337777773333333333}
            NumGlyphs = 2
          end
          object BBtnCancelar: TBitBtn
            Left = 2
            Top = 42
            Width = 103
            Height = 40
            Cursor = crHandPoint
            Caption = '&Cancelar'
            TabOrder = 1
            OnClick = BBtnCancelarClick
            Glyph.Data = {
              
              3333333777333777333333333999993333333333377777333333}
            NumGlyphs = 2
          end
        end
        object DBEdit1: TDBEdit
          Left = 16
          Top = 40
          Width = 529
          Height = 24
          CharCase = ecUpperCase
          DataField = 'NomeProfSolic'
          DataSource = DSQualquer
          TabOrder = 0
        end
        object DBEdit2: TDBEdit
          Left = 168
          Top = 87
          Width = 199
          Height = 24
          CharCase = ecUpperCase
          DataField = 'NumConselho'
          DataSource = DSQualquer
          TabOrder = 2
        end
        object RxDBLookupCombo1: TRxDBLookupCombo
          Left = 16
          Top = 88
          Width = 145
          Height = 23
          DropDownCount = 8
          DataField = 'AbrevConselho'
          DataSource = DSQualquer
          LookupField = 'AbrevConselho'
          LookupDisplay = 'AbrevConselho'
          LookupSource = DSConselho
          TabOrder = 1
        end
        object DBEdit3: TDBEdit
          Left = 16
          Top = 176
          Width = 529
          Height = 24
          CharCase = ecLowerCase
          DataField = 'Email'
          DataSource = DSQualquer
          TabOrder = 6
        end
        object DBEdit4: TDBEdit
          Left = 16
          Top = 131
          Width = 176
          Height = 24
          DataField = 'Telefone'
          DataSource = DSQualquer
          TabOrder = 4
        end
        object DBEdit5: TDBEdit
          Left = 200
          Top = 131
          Width = 137
          Height = 24
          DataField = 'Celular'
          DataSource = DSQualquer
          TabOrder = 5
        end
        object DBDateEdit1: TDBDateEdit
          Left = 376
          Top = 87
          Width = 121
          Height = 21
          DataField = 'Nascimento'
          DataSource = DSQualquer
          NumGlyphs = 2
          TabOrder = 3
        end
        object DBCheckBox1: TDBCheckBox
          Left = 400
          Top = 8
          Width = 97
          Height = 17
          Caption = 'Ativo'
          DataField = 'ativo'
          DataSource = DSQualquer
          Font.Charset = ANSI_CHARSET
          Font.Color = clRed
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          ParentFont = False
          TabOrder = 7
          ValueChecked = 'S'
          ValueUnchecked = 'N'
        end
      end
    end
  end
  object QryQualquer: TQuery
    CachedUpdates = True
    DatabaseName = 'DNLABVET'
    SQL.Strings = (
      'Select *'
      'From ProfissionalSolicitante'
      'Where NomeProfSolic Like :Nome'
      'and'
      '( Ativo = :status or :status = '#39'-1'#39')'
      'Order by NomeProfSolic')
    UpdateObject = UDPQualquer
    Left = 341
    Top = 125
    ParamData = <
      item
        DataType = ftString
        Name = 'Nome'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'status'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'status'
        ParamType = ptInput
      end>
    object QryQualquerCodProfSolic: TAutoIncField
      FieldName = 'CodProfSolic'
      Origin = 'DNLABVET.ProfissionalSolicitante.CodProfSolic'
    end
    object QryQualquerAbrevConselho: TStringField
      FieldName = 'AbrevConselho'
      Origin = 'DNLABVET.ProfissionalSolicitante.AbrevConselho'
      FixedChar = True
      Size = 10
    end
    object QryQualquerNomeProfSolic: TStringField
      FieldName = 'NomeProfSolic'
      Origin = 'DNLABVET.ProfissionalSolicitante.NomeProfSolic'
      FixedChar = True
      Size = 50
    end
    object QryQualquerNumConselho: TStringField
      FieldName = 'NumConselho'
      Origin = 'DNLABVET.ProfissionalSolicitante.NumConselho'
      FixedChar = True
      Size = 15
    end
    object QryQualquerEmail: TStringField
      FieldName = 'Email'
      Origin = 'DNLABVET.ProfissionalSolicitante.Email'
      FixedChar = True
      Size = 200
    end
    object QryQualquerTelefone: TStringField
      FieldName = 'Telefone'
      Origin = 'DNLABVET.ProfissionalSolicitante.Telefone'
      EditMask = '!\(99\)0000-0000;1;_'
      FixedChar = True
      Size = 15
    end
    object QryQualquerCelular: TStringField
      FieldName = 'Celular'
      Origin = 'DNLABVET.ProfissionalSolicitante.Celular'
      EditMask = '!\(99\)0000-0000;1;_'
      FixedChar = True
      Size = 15
    end
    object QryQualquerNascimento: TDateTimeField
      FieldName = 'Nascimento'
      Origin = 'DNLABVET.ProfissionalSolicitante.Nascimento'
    end
    object QryQualquerativo: TStringField
      FieldName = 'ativo'
      Origin = 'DNLABVET.ProfissionalSolicitante.ativo'
      FixedChar = True
      Size = 1
    end
  end
  object UDPQualquer: TUpdateSQL
    ModifySQL.Strings = (
      'update ProfissionalSolicitante'
      'set'
      '  AbrevConselho = :AbrevConselho,'
      '  NomeProfSolic = :NomeProfSolic,'
      '  NumConselho = :NumConselho,'
      '  Email = :Email,'
      '  Telefone = :Telefone,'
      '  Celular = :Celular,'
      '  Nascimento = :Nascimento,'
      '  ativo = :ativo'
      'where'
      '  CodProfSolic = :OLD_CodProfSolic')
    InsertSQL.Strings = (
      'insert into ProfissionalSolicitante'
      
        '  (AbrevConselho, NomeProfSolic, NumConselho, Email, Telefone, C' +
        'elular, '
      '   Nascimento, ativo)'
      'values'
      
        '  (:AbrevConselho, :NomeProfSolic, :NumConselho, :Email, :Telefo' +
        'ne, :Celular, '
      '   :Nascimento, :ativo)')
    DeleteSQL.Strings = (
      'delete from ProfissionalSolicitante'
      'where'
      '  CodProfSolic = :OLD_CodProfSolic')
    Left = 389
    Top = 165
  end
  object DSQualquer: TDataSource
    AutoEdit = False
    DataSet = QryQualquer
    Left = 421
    Top = 125
  end
  object QryConselho: TQuery
    DatabaseName = 'DNLABVET'
    SQL.Strings = (
      'Select *'
      'From Conselho'
      'Order by NomeConselho')
    Left = 349
    Top = 234
    object QryConselhoAbrevConselho: TStringField
      FieldName = 'AbrevConselho'
      Origin = 'DNLABVET.Conselho.AbrevConselho'
      FixedChar = True
      Size = 10
    end
    object QryConselhoNomeConselho: TStringField
      FieldName = 'NomeConselho'
      Origin = 'DNLABVET.Conselho.NomeConselho'
      FixedChar = True
      Size = 50
    end
  end
  object DSConselho: TDataSource
    AutoEdit = False
    DataSet = QryConselho
    Left = 389
    Top = 234
  end
end
