...
procedure TFrmRelDesconto.QRBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
   VlCalc := VlCalc + QryContasVLCALCULADO.Value;
   VlDesc := VLDesc + QryContasVLDESCONTO.Value;
   VLFat := VLFat + QryContasVLFATURA.Value;
   VLPag := VLPag + QryContasVLPGTO.Value;

   VltCalc := VltCalc + QryContasVLCALCULADO.Value;
   VltDesc := VLtDesc + QryContasVLDESCONTO.Value;
   VLtFat := VLtFat + QryContasVLFATURA.Value;
   VLtPag := VLtPag + QryContasVLPGTO.Value;
